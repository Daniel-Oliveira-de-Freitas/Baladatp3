...
public class ClienteController {
	@FXML
	private MenuButton categoriafield, categoriaAlterar, valoresfield;
	@FXML
	...
	@FXML
	private MenuItem item1, item2, item3, item7, item8, item9;
	@FXML
	...
	public void procuraCliente() {
		...
		if (telacredito.isSelected() && !barraPesquisacredito.getText().isEmpty()) {
			...
			alertMessage("Cliente inexistente");
		}
	}
	...
	public void cadastroCliente() {
		...
		if (!rgfield.getText().isEmpty() && !nomefield.getText().isEmpty() && !categoriafield.getText().isEmpty()
			...
			if(categoriafield.getText().equals("VIP")) {
				Cliente c = new Cliente(rg, nome, categoria, credito);
				Main.instancia().AdicionarCliente(c);
				Main.instancia().salvarCliente();
				alertMessage("Salvo com sucesso");
				acaobotaovoltar();
			}else if(!categoriafield.getText().equals("VIP") && credito>0) {
			Cliente c = new Cliente(rg, nome, categoria, credito);
			Main.instancia().AdicionarCliente(c);
			Main.instancia().salvarCliente();
			alertMessage("Salvo com sucesso");
			acaobotaovoltar();
			}else {alertMessage("Credito deve ser meio que 0");}
		} else {
	...
	public void deleteCliente() {
		...
		try {	
			...
			confirmacao.setContentText("Você deseja excluir o cliente selecionado?");
			...
			alertMessage("Não há mais clientes para deletar");
		}catch(Exception a) {
